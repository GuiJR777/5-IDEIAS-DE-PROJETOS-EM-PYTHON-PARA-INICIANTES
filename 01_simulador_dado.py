...
def lanca_dado(tipo, quantidade_de_dados):
    valor_max = int(tipo)
    lancamentos = int(quantidade_de_dados)
    resultados = []
    if lancamentos > 1:
...
while lancar == True:
    ...
    try:
        ...
        if resposta == 1:
            ...
            sleep(2)
            print(f'Você vai lançar o D{ dados[tipo] }')
            sleep(1)
            # Seleção de quantidade
            print('Quantos dados você quer jogar?')
            sleep(1)
            try:
                qtd = int(input('Sua resposta(deve ser um numero inteiro): '))
            except:
                print('Resposta invalida, tente de novo')
            resultado = lanca_dado(dados[tipo], qtd)
            if type(resultado) == list:
                resultado = soma_resultado(resultado)
            print(f'Você lançou {qtd} D{dados[tipo]} e obteve {resultado} como resultado')
            sleep(10)
